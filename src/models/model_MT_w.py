...
def add_constraints_w(mdl, I, x, y, r, w, s, a, b, D, U, z, kappa, h):
    """
...
def create_MT_model_w(I: Instance):
    ...
    # 2) constraints
    add_constraints_w(mdl, I, x, y, r, w, s, a, b, D, U, z, kappa,h)

    # 3) objective
    add_objective_w(mdl, I, h, s)
...
######################################################################
### Version with is_Nw_used (Nw active only for some time windows) ###
######################################################################

# Create active windows for Nw
def build_active_sets(I: Instance, is_Nw_used: dict):
    Nw = I.Nw       # Nw nodes list
    T = I.T         # list of discrete times (i.e. time windows)
    Nw_active_t = {t: set() for t in T}     # Dictionary for each t: set of active nodes at that time t 
    IwT_active = set()                      # Set of active couples (i,t)

    for (i, t), val in is_Nw_used.items():   # For each key and value of is_Nw_used
        if val == 1:
            Nw_active_t[t].add(i)      # Add node i at the set for time t
            IwT_active.add((i, t))     # Add the active couple (i,t)

    return Nw_active_t, IwT_active





def create_decision_variables_w_Nw(mdl: Model, I: Instance, Nw_active_t: dict):
    """
    Create all MILP decision variables for the taxi-like model.
    """
    M = I.M
    ...
    ### NEW: w exists only for active (nw, t) ###
    # w[k,i,t,m,mp] only for:
    #   - t ∈ ΔT_k (request is "alive")
    #   - i ∈ Nw (internal nodes where swaps are allowed)
    # w SOLO dove (i,t) attivo
    w_keys = []
    for k in I.K:
        tmin_k = min(I.DeltaT[k])
        for t in I.DeltaT[k]:
            if t == tmin_k:
                continue
            for i in Nw_active_t.get(t, ()):
                for m in M:
                    for mp in M:
                        if m != mp:
                            w_keys.append((k,i,t,m,mp))
    ...
    ### NEW: D always existing, but will be 0 for inactive t ###
    # D[m,i,t] = n° TRAIL rilasciati dal MAIN m nel nodo i al tempo t
    D = mdl.integer_var_dict(
        keys=[(m, i, t)
              for m in M
    ...
    ### NEW: U always existing, but will be 0 for inactive t ###
    # U[m,i,t] = n° TRAIL presi dal MAIN m nel nodo i al tempo t
    U = mdl.integer_var_dict(
        keys=[(m, i, t)
              for m in M
...
def add_constraints_w_Nw(mdl, I, is_Nw_used, Nw_active_t, x, y, r, w, s, a, b, D, U, z, kappa, h):
    """
    Add all constraints of the taxi-like MILP model to the docplex model.
    """
    N = I.N
    ...
    # 5.NEW) Spegnimento per U e D per (nw, t) non attivi:
    for t in I.T[1:]:
        for i in I.Nw:
            if is_Nw_used[(i,t)] == 0:
                for m in I.M:
                    mdl.add_constraint(D[m,i,t] == 0, ctname=f"D_off_m{m}_i{i}_t{t}")
                    mdl.add_constraint(U[m,i,t] == 0, ctname=f"U_off_m{m}_i{i}_t{t}")
    ...
    # (d.NEW) At most 1 swap per k,t
    # Only w for active (nw, t)
    for k in K:
        tmin = min(DeltaT[k])
        for t in DeltaT[k]:
            if t == tmin:   # non essitono w per t_min
            ...
            mdl.add_constraint(
                mdl.sum(
                    w[k, i, t, m, mp]
                    for i in Nw_active_t.get(t, ())    # only where w exixsts
                    for m in M
                    for mp in M
                    if m != mp and (k, i, t, m, mp) in w
                ) <= 1,
    ...
    # (10.2.NEW) Dinamica di r tramite a,b e w
    # Non esistono w per (nw,t) inattivi!
    for k in K:
        times_k = sorted(DeltaT[k])
        if not times_k:
            continue
        ...
        for m in M:
            ...
            for t_prev, t in zip(times_k[:-1], times_k[1:]):
                ...
                # scambio "in uscita" dal modulo m al tempo t
                swap_out = mdl.sum(                # Non esistono w per (nw,t) inattivi!
                    w[k, i, t, m, mp]
                    for i in Nw_active_t.get(t, ())     # only active t
                    for mp in M
                    if mp != m and (k, i, t, m, mp) in w
                )

                # scambio "in entrata" sul modulo m al tempo t
                swap_in = mdl.sum(                  # Non esistono w per (nw,t) inattivi!
                    w[k, i, t, mp, m]
                    for i in Nw_active_t.get(t, ())     # only active t
                    for mp in M
                    if mp != m and (k, i, t, mp, m) in w
...
def create_MT_model_w_Nw(I: Instance, is_Nw_used: dict):
    """
    Create:
        - Model()
        - decision variables
    ...
    Role of is_Nw_used:
        - kappa exists always
        - U/D are 0 if (nw,t) is inactive 
        - w does not exist if (nw,t) is inactive
    """

    mdl = Model(name="Main-Trail-Nw")

    Nw_active_t, IwT_active = build_active_sets(I, is_Nw_used)

    # 1) variables
    x, y, r, w, s, a, b, D, U, z, kappa,h = create_decision_variables_w_Nw(mdl, I, Nw_active_t)

    # 2) constraints
    add_constraints_w_Nw(mdl, I, is_Nw_used, Nw_active_t, x, y, r, w, s, a, b, D, U, z, kappa,h)
