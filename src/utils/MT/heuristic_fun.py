...
def run_heuristic(
    *,
    ...
    number: int = 3,
    # CITY params
    city: str = "Torino, Italia",
    subdir: str = "TORINO_SUB",
    central_suburbs: list[str] | None = None,
    # heuristic params
    KEEP: int = 4,
    FICT: int = 5,
    ...
    """
    Runs the request-reduction heuristic and saves a JSON with reduced requests.

    Returns
    -------
    dict with:
      - out_path: Path to requests_REDUCED.json
      - fixed_k: list of fixed request ids
      - fict_ids: list of fictitious request ids
      - network_path, requests_path: generated original paths
    """
    if central_suburbs is None:
        central_suburbs = ["Centro", "Crocetta", "Santa Rita", "Aurora"]

    MODE = MODE.upper().strip()
    if MODE not in {"GRID", "CITY"}:
        raise ValueError(f"MODE must be 'GRID' or 'CITY', got: {MODE}")
    ...
    # output base
    base_output_folder = Path(base_results_dir) / f"{MODE}_h{horizon}_dt{dt}_K{num_requests}_seed{seed}"
    ...
    if MODE == "GRID":
        ...
        # Qui serve il path "continuous" equivalente (come fai per GRID).
        # Se non ce l'hai ancora, devi crearlo/decidere dove sta.
        # Per ora assumo che tu abbia già questi due file:
        network_cont_path = f"instances/CITY/{subdir}/network.json"
        requests_cont_path = f"instances/CITY/{subdir}/taxi_like_requests_{horizon}maxmin.json"

        G = load_network_continuous_as_graph(network_cont_path)
        req4d = build_requests_4d_from_file(requests_cont_path)

    # -----------------------------
    # EMBEDDING + 6D features
    # -----------------------------
    node_xy = mds_embed_nodes_from_sp(G, weight="time_min", symmetrize="avg", dim=2)
    req6d = build_requests_6d_from_4d(req4d, node_xy)

    # -----------------------------
    # SELECT FIXED + CREATE FICT
    # -----------------------------
    steps = iterative_remove_by_centroid(
        req6d,
        n_remove=KEEP,
        use_capacity=use_capacity,
        capacity_key="q",
    ...
    fixed_k = [s["removed_k"] for s in steps]
    ...
    # -----------------------------
    # SAVE JSON (discrete-generator compatible format)
    # -----------------------------
    ...
    return {
        "out_path": out_path,
        "fixed_k": fixed_k,
        "fict_ids": [f["k"] for f in fict_graph],
...
if __name__ == "__main__":
    ...
    print("Saved reduced requests to:", res["out_path"])